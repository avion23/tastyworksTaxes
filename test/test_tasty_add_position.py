...
class TestTastyAddPosition:
    def test_lfin_calls_open_and_close(self):
        ...
        assert position.getSymbol() == 'LFIN'
        assert position.getQuantity() == 2
        ...
        position = Transaction(t.positions.squeeze())
        assert position.getQuantity() == 1
        assert t.closedTrades.squeeze()['Quantity'] == 1
    ...
    def test_sprt_position(self):
        ...
        position = Transaction(t.positions.squeeze())
        assert position.getSymbol() == 'SPRT'
        assert position.getQuantity() == 200
        assert position.getValue().usd == -774